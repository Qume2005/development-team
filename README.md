...
> 一个让 AI agent 化身 IT 团队经理的 Claude Code skill — 用 17 个专业角色协作完成软件工程任务，同时守护你宝贵的 context window。
...
**The Company** 正是为了解决这个问题而诞生的。它的核心理念很简单：让 AI agent 扮演一位 IT 团队经理，**经理从不亲自动手** — 它只负责理解需求、设计工作流、派遣专业子角色（subagent）执行，然后根据子角色返回的简要裁决（1-2 句话摘要）做出决策。所有上下文通过磁盘上的结构化文档流转，经理永远不直接阅读交付物本身。

这不是一个代码库，也不是一个框架。它是一组精心设计的 Markdown 规则文件（共 19 个），安装到 `~/.claude/skills/` 目录后，就能让 Claude Code 的每一次对话自动进入「经理模式」。无论你是要构建一个全新的全栈项目，还是修一个小 bug，The Company 都会自动匹配最合适的工作流模板，以 TDD、代码审查、质量门控的方式交付高质量的成果。
...
| **经理从不亲自动手** | Manager 永远不读代码、不写文档、不跑命令 — 所有实际工作委派给 subagent |
| **磁盘是通信管道** | 子角色之间不通过对话传递上下文，而是通过 `.claude/the-company/` 目录下的结构化交付文档 |
| **只吸收裁决** | Manager 只接收文件路径 + 1-2 句话的结论，用于决策，绝不阅读完整交付物 |
...
| 角色 | 文件 | 职责 |
|------|------|------|
| Manager（经理） | `SKILL.md` | 理解需求、设计工作流、派遣子角色、做决策 — 但从不亲自动手 |
...
每个模板中，生产角色的交付物都会经过配对审查角色的审核，审查通过后才进入下一步。Manager 会根据任务的实际情况选择合适的模板，也可以自定义流程。
...
安装完成后，Claude Code 的每次对话都会自动激活 The Company skill。Manager 会自动接管，分析你的请求并提议工作流。
...
```
the-company/
├── SKILL.md                     # 入口 / Manager 规则（Skill 清单）
...
这是一个个人项目，目前由一位开发者独立维护。如果你有建议或发现 bug，欢迎通过 [GitHub Issues](https://github.com/your-username/the-company/issues) 反馈。

## 致谢

The Company 的设计灵感来自于一个朴素的观察：**AI agent 的 context window 是最宝贵的资源**，而大多数交互模式都在无意识地挥霍它。感谢 Anthropic 的 Claude Code 团队提供了 skill 系统和 subagent 机制，让这种「经理模式」成为可能。
